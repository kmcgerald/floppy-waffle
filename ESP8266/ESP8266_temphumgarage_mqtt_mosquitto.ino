...
void loop() {
  ...
  int tempF_data = (int)dht.readTemperature(true);
  int tempC_data = (int)dht.readTemperature();
  ...
  if (now - lastMsg > 5000) {
    ...
    client.publish("sensors/farenheit/", msg);
    
    dtostrf(tempC_data, 4, 2, msg);
    client.publish("sensors/celcius/", msg);
  }
